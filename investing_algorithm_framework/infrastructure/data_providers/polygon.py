...
class PolygonOHLCVDataProvider(DataProvider):
    """
    ...
    def has_data(
        ...
        if tf not in TIMEFRAME_TO_POLYGON:
            return False
    ...
    def prepare_backtest_data(
        ...
        if data is None:
            if show_progress:
                logger.info(
                    f"Downloading Polygon.io data for {self.symbol} "
        ...
        if self.data is None or len(self.data) == 0:
            raise OperationalException(
                ...
                f"Please ensure the symbol is valid on Polygon.io."
            )
    ...
    def copy(self, data_source) -> "PolygonOHLCVDataProvider":
    ...
    def _get_api_key(self) -> str:
        credential = self.get_credential("POLYGON")

        if credential is not None:
            return credential.api_key

        raise OperationalException(
            "Polygon.io requires an API key. "
            "Configure it with: app.add_market_credential("
            "MarketCredential(market='POLYGON', "
            "api_key='your_key'))"
        )

    def _get_polygon_config(self) -> tuple:
        tf = self.time_frame

        if hasattr(tf, "value"):
            tf = tf.value

        config = TIMEFRAME_TO_POLYGON.get(tf)

        if config is None:
            raise OperationalException(
                f"Time frame '{tf}' is not supported by Polygon.io. "
                f"Supported time frames: "
                f"{list(TIMEFRAME_TO_POLYGON.keys())}"
            )
    ...
    def _download_ohlcv(
        ...
        polygon_config = TIMEFRAME_TO_POLYGON.get(tf)
        ...
        timespan, multiplier = polygon_config
    ...
    def _get_data_from_storage(
        ...
        file_name = f"{safe_symbol}_{tf}_polygon.csv"
        ...
        try:
            ...
            logger.error(
                f"Error reading Polygon.io data from storage: {e}"
            )
            return None
