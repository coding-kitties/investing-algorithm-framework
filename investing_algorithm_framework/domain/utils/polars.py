...
def convert_polars_to_pandas(data: PolarsDataFrame, remove_duplicates = True, add_index=True, datetime_column_name = "Datetime"):
    """
    Function to convert polars dataframe to pandas dataframe. 

    Parameters:
        data:Polars Dataframe - The original polars dataframe 
        remove_duplicates: Boolean - If set to true, all duplicate dates will be removed from the dataframe
        add_index: Boolean - If set to true, an index will be added to the dataframe
        datetime_column_name: String - the column name that has the datetime object. By default this is set to column name Datetime
