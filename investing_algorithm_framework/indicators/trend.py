...
"""
This module contains functions for trend analysis. Trend analysis is the process of analyzing the direction 
of the price of an asset.
or the direction of the market as a whole. This is done by analyzing the moving averages of the price data or 
indicators such as the RSI.
"""

def is_uptrend(data: Union[pd.DataFrame, pd.Series], fast_key="SMA_50", slow_key="SMA_200") -> bool:
    """
    ...
    if not isinstance(data, pd.Series) and not isinstance(data, pd.DataFrame):
        raise OperationalException("Provided data must be of type pandas series or pandas dataframe")
...
def is_downtrend(data: Union[pd.DataFrame, pd.Series], fast_key="SMA_50", slow_key="SMA_200") -> bool:
    """
...
def has_crossed_upward(data: pd.DataFrame, key, threshold, strict=True) -> bool:
    """
    ...
    Returns:
        - Boolean indicating if the key has crossed upward through the threshold within the given data frame.
    """
...
def get_up_and_downtrends(data: pd.DataFrame) -> List[DateRange]:
    ...
    Params:
        data: pd.Dataframe - instance of pandas Dateframe containing OHLCV data.
    
    Returns:
        List of date ranges that with up_trend and down_trend flags specified.
    """

    # Check if the data is larger then 200 data points
    if len(data) < 200:
        raise OperationalException("The data must be larger than 200 data points to determine up and down trends.")
    ...
    for idx, row in enumerate(selection.itertuples(index=True), start=1):
        ...
        if pd.isnull(selected_rows["SMA_Close_50"].iloc[-1]) or pd.isnull(selected_rows["SMA_Close_200"].iloc[-1]):
            continue

        if is_uptrend(selected_rows, fast_key="SMA_Close_50", slow_key="SMA_Close_200"):
            
            if current_trend != 'Up':

                if current_trend is not None:
                    end_date = selection.loc[row.Index - timedelta(days=1)].name
                    date_ranges.append(DateRange(start_date=start_date_range, end_date=end_date, name=current_trend, down_trend=True))
            ...
            if current_trend != 'Down':
                ...
                if current_trend is not None:
                    end_date = selection.loc[row.Index - timedelta(days=1)].name
                    date_ranges.append(DateRange(start_date=start_date_range, end_date=end_date, name=current_trend, up_trend=True))
                    start_date_range = row.Index
                    current_trend = 'Down'
                else:
                    current_trend = 'Down'
...
def get_sma(data: pd.DataFrame, period=50, source_column_name="Close", result_column_name = None):
    """
    Function to add Smoothed moving average to a pandas dataframe.

    Params:
        data: pd.Dataframe - instance of pandas Dateframe.
        period: int - the number of data points the SMA needs to take into account.
        source_column_name: str - the source_column_name that will be used to calculate the SMA. 
        result_column_name: (option) str - if set, this will be used as column in the 
        dataframe where the result will be written to. If not set the result column is 
        named 'SMA_{key}_{period}'.
    
    Returns:
        Pandas dataframe with SMA column added, named 'sma_{key}_{period}' or name 
        according to the result_column_name
    """
    # Check if the period is larger than the data
    if period > len(data):
        raise OperationalException(f"The period {period} is larger than the data.")
    
    sma_values = tp.sma(data[source_column_name].to_numpy(), period=period)

    # Pad NaN values for initial rows with a default value, e.g., 0
    sma_values = np.concatenate((np.full(period - 1, None), sma_values))
