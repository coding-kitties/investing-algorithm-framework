...
class TestStrategy(TradingStrategy):
    ...
    def apply_strategy(self, algorithm, market_data):
        for symbol in self.symbols:
            ...
            if self.is_uptrend(ohlcv_df):

                if self.is_buy_signal_uptrend(
                ...
                elif self.is_sell_signal_uptrend(
                    symbol=target_symbol, algorithm=algorithm, data=ohlcv_df
                ):
                    algorithm.close_position(
                        symbol=target_symbol
                )
            elif self.is_downtrend(ohlcv_df):
                
                if self.is_buy_signal_downtrend(
                    symbol=target_symbol, algorithm=algorithm, data=ohlcv_df
                ):
                    algorithm.create_limit_order(
                        target_symbol=target_symbol,
                ...
                elif self.is_sell_signal_downtrend(
                    symbol=target_symbol, algorithm=algorithm, data=ohlcv_df
                ):
                    algorithm.close_position(
                        symbol=target_symbol
    ...
    def is_uptrend(self, data):
        return self.is_above(data, "EMA_50", "EMA_100")
    
    def is_downtrend(self, data):
        return self.is_below(data, "EMA_50", "EMA_100")
    
    def is_buy_signal_uptrend(self, symbol, algorithm, data):
        """
        Buy Signal on uptrend:
        - The +DI crosses above the -DI + one of the following
        conditions is met:
        ...
        if self.is_crossover(data, "+DI", "-DI"):
            ...
            close_window = data.loc[
                row.name - pd.Timedelta(days=2):row.name, 'Close_lows'
            ]
    ...
    def is_buy_signal_downtrend(self, symbol, algorithm, data):
        """
        Buy Signal on uptrend:
        - The +DI crosses above the -DI + one of the following
        conditions is met:
    ...
    def is_sell_signal_uptrend(self, symbol, algorithm, data):
        """
        Sell Signal on uptrend:
        - The -DI crosses above the +DI + one of the following conditions
        is met:
        ...
        if self.is_crossover(data, "-DI", "+DI"):
            ...
            # and the close price lows in the last 2 days
            rsi_window = data.loc[
                row.name - pd.Timedelta(days=2):row.name, 'RSI_highs'
            ]
            close_window = data.loc[
                row.name - pd.Timedelta(days=2):row.name, 'Close_highs'
            ]

            # Go over each row and check if there is a bearish
    ...
    def is_sell_signal_downtrend(self, symbol, algorithm, data):
        """
        Sell Signal on downtrend:
        - The +DI crosses above the -DI + one of the following conditions
        is met:
            - If in the last 7 days there is a bullish divergence
                (higher low and lower low) between the RSI_lows and
                the close price lows
            - If the RSI has decreased significantly in the last day
                (RSI_diff < -2)
        ...
        if self.is_crossover(data, "-DI", "+DI"):
            ...
            # and the close price lows in the last 2 days
            rsi_window = data.loc[
                row.name - pd.Timedelta(days=7):row.name, 'RSI_highs'
            ]
            close_window = data.loc[
                row.name - pd.Timedelta(days=7):row.name, 'Close_highs'
            ]
