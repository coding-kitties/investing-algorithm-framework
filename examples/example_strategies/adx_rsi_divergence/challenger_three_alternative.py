...
"""
This is a challenger strategy that uses RSI and ADX metrics. The strategy
is parameterized in order to do experiments with it. This allows you
to test the strategy with different configurations.

The strategy calculates the CLOSE and RSI peaks and uses them to identify
bearish and bullish divergences. A bullish divergence is identified when
the RSI makes a higher low while the close price makes a lower low. 
A bearish divergence is identified when the RSI makes a lower high while
the close price makes a higher high.

A buy signal is created when the +DI crosses above the -DI and 
withing the last 2 days there is a bullish divergence between the RSI
and the close price or the RSI has decreased significantly in the last day.

A sell signal is created when the -DI crosses above the +DI and 
withing the last 2 days there is a bearish divergence between the RSI
and the close price or the ADX has decreased significantly in the last day.

The strategy also checks for manual stop losses. If the price drops by 6%
from the entry price, the trade is closed. 
"""
...
class TestStrategy(TradingStrategy):
    ...
    Buy Signal:
        - There is a bullish divergence (higher low and lower low) between
            the +DI and the -DI
        - If the RSI has decreased significantly in the last days
            (RSI_diff < -2)

    Sell Signal:
        - The -DI crosses above the +DI
        - ADX has decreased significantly in the last day
            (ADX_diff < -1)
    ...
    def apply_strategy(self, algorithm, market_data):
        for symbol in self.symbols:
            ...
            ohlcv_df = self.add_peaks(
                ohlcv_df, key="+DI", order=self.peaks_order, k=self.peaks_k
            )
            ohlcv_df = self.add_peaks(
                ohlcv_df, key="-DI", order=self.peaks_order, k=self.peaks_k
            )
            ...
            if self.is_buy_signal(symbol=target_symbol, algorithm=algorithm, data=ohlcv_df):
            ...
            elif self.is_sell_signal(symbol=target_symbol, algorithm=algorithm, data=ohlcv_df):
    ...
    def is_buy_signal(self, symbol, algorithm, data):
        ...
        # Check if there is a bullish divergence between the +DI and -DI
        di_plus_low = row['+DI_lows']
        di_minus_low = row['-DI_lows']

        if di_plus_low == -1 and di_minus_low == 1:
    ...
    def is_crossover(self, data, key1, key2):
        ...
        """
        return data[key1].iloc[-1] > data[key2].iloc[-1] and data[key1].iloc[-2] < data[key2].iloc[-2]
    ...
    def add_peaks(data, key="Close", order=5, k=2):
        ...
        consider when calculating the peaks. If the order is 2, the function will consider
        ...
        The K parameter determines how many consecutive peaks need to be higher or lower to be considered a peak.
        """
    ...
    def get_lower_lows(data: np.array, order=5, K=2):
        ...
        K : int, optional
        How many consecutive lows need to be lower. This means that for a given low, the next
        K lows must be lower than the k lows before. So say K=2, then the low at index i must be lower than the low at index i-2 and i-1. If this
        condition is met, then the low at index i is considered a lower low. If the condition is not met, then the low at index i is not considered a lower low.
        '''
        # Get lows
        low_idx = argrelextrema(data, np.less, order=order)[0]
        lows = data[low_idx]
    ...
    def add_adx(data, period=14):
        plus_di, min_di = tp.di(high=data["High"].to_numpy(), low=data["Low"].to_numpy(), close=data["Close"].to_numpy(), period=period)
        adx = tp.adx(high=data["High"].to_numpy(), low=data["Low"].to_numpy(), close=data["Close"].to_numpy(), period=period)
