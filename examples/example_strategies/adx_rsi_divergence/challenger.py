...
class RSIADXCrossOverAndDivergenceStrategy(TradingStrategy):
    ...
    def apply_strategy(self, algorithm, market_data):
        ...
        for symbol in self.symbols:
            ...
            ohlcv_df = ohlcv_df.to_pandas()
            ohlcv_df = self.add_rsi(ohlcv_df)
            ohlcv_df = self.add_adx(ohlcv_df)
    ...
    def is_buy_signal(self, symbol, algorithm, data):
        ...
        if current_row["ADX"] >= self.adx_buy_threshold \
                and self.is_crossover(data["+DI"], data["-DI"]) \
                and current_row["RSI"] <= self.rsi_buy_threshold:
            return True
    ...
    def is_sell_signal(self, symbol, algorithm, data):
        ...
        if current_row["ADX"] > self.adx_sell_threshold and self.is_bearish_divergence(data) and current_row["RSI"] >= self.rsi_sell_threshold:
             return True
