...
class TestBacktestSaveOpen(unittest.TestCase):
    ...
    def test_open_with_backtest_date_ranges(self):
        ...
        backtest.save(self.dir_path)
        # Load all runs (no filter)
        loaded_backtest = Backtest.open(self.dir_path)
        self.assertEqual(len(loaded_backtest.get_all_backtest_runs()), 2)
        # Load only the first backtest run by date range
        loaded_backtest_one = Backtest.open(
            self.dir_path,
            backtest_date_ranges=[backtest_date_range_one]
        )
        self.assertEqual(len(loaded_backtest_one.get_all_backtest_runs()), 1)
        run = loaded_backtest_one.get_all_backtest_runs()[0]
        self.assertEqual(run.backtest_start_date, backtest_date_range_one.start_date)
        self.assertEqual(run.backtest_end_date, backtest_date_range_one.end_date)
        self.assertEqual(run.backtest_date_range_name, backtest_date_range_one.name)
        # Load only the second backtest run by date range
        loaded_backtest_two = Backtest.open(
            self.dir_path,
            backtest_date_ranges=[backtest_date_range_two]
        )
        self.assertEqual(len(loaded_backtest_two.get_all_backtest_runs()), 1)
        run2 = loaded_backtest_two.get_all_backtest_runs()[0]
        self.assertEqual(run2.backtest_start_date, backtest_date_range_two.start_date)
        self.assertEqual(run2.backtest_end_date, backtest_date_range_two.end_date)
        self.assertEqual(run2.backtest_date_range_name, backtest_date_range_two.name)
    ...
    def test_backtest_set_uniqueness_by_metadata_id(self):
        # Create two Backtest instances with the same metadata["id"]
        ...
        # Create a set and check uniqueness
        backtest_set = {backtest1, backtest2, backtest3}
        self.assertEqual(len(backtest_set), 2)
        self.assertIn(backtest1, backtest_set)
        self.assertIn(backtest3, backtest_set)
        # backtest2 should be considered equal to backtest1 (same metadata["id"])
        self.assertIn(backtest2, backtest_set)
        ...
        backtest_set2 = {backtest4, backtest5}
