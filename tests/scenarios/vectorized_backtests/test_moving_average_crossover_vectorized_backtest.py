...
class MovingAverageCrossoverVectorizedBacktest(TradingStrategy):
    ...
    def buy_signal_vectorized(self, data: Dict[DataSource, Any]) -> pd.Series:
        ...
        macd_data_source = next(
            (ds for ds in self.data_sources if ds.time_frame.equals(self.macd_time_frame) and DataType.OHLCV.equals(ds.data_type)),
            None
        )
        ema_data_source = next(
            (ds for ds in self.data_sources if ds.time_frame.equals(self.ema_time_frame) and DataType.OHLCV.equals(ds.data_type)),
            None
        )

        macd_df = data[macd_data_source].copy()
        ema_df = data[ema_data_source].copy()
...
class Test(TestCase):
    ...
    def test_run(self):
        ...
        self.assertIsNotNone(backtest_results.growth)
        self.assertIsNotNone(backtest_results.growth_percentage)
        self.assertIsNotNone(backtest_results.initial_unallocated)
        self.assertIsNotNone(backtest_results.trading_symbol)
        self.assertIsNotNone(backtest_results.total_net_gain)
        self.assertIsNotNone(backtest_results.total_net_gain_percentage)
        self.assertIsNotNone(backtest_results.get_portfolio_snapshots())


        # self.assertAlmostEqual(
        #     backtest_report.backtest_results.growth, 3.6, delta=0.5
        # )
        # self.assertAlmostEqual(
        #     backtest_report.backtest_results.growth_percentage, 0.9, delta=0.1
        # )
        # self.assertEqual(
        ...
        # )
        # self.assertAlmostEqual(
        #     backtest_report.backtest_results.total_net_gain, 3.6, delta=0.5
        # )
        # self.assertAlmostEqual(
        #     backtest_report.backtest_results.total_net_gain_percentage, 0.9, delta=0.1
        # )
