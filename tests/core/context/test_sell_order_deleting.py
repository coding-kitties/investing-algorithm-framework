...
class TestOrderModel(TestBase):
    ...
    def test_order_creation(self):
        ...
        self.assertEqual(
            1, Order.query.filter_by(order_type=OrderType.SELL.value).count()
        )
        self.assertEqual(
            1, Position.query.filter_by(symbol="BTC").count()
        )
    ...
    def test_order_creation_without_matching_portfolio_manager(self):
        ...
        self.assertEqual(
            0, Order.query.filter_by(order_type=OrderType.SELL.value).count()
        )
        self.assertEqual(
            0, Position.query.filter_by(symbol="BTC").count()
        )
